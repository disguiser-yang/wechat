$(function() {
    ...
    if (getQueryString('edit') == 'true') {
        $('.delete-address').show();
        document.title = '编辑收货地址'
    } else {
        document.title = '新添收货地址';
        $('#top-mui-table-view').css('marginTop', '15px');
    }
});
...
// mui('body').on('tap', '.delete-address', function() {
...
mui('body').on('tap', '.delete-address', function() {
    ...
    mui.confirm('确定删除该地址吗？', ' ', ['取消', '确定'], function(result) {
        if (result.index == 1) {
            appui.showHUD();
            window.IO({
                url: 'logistics',
