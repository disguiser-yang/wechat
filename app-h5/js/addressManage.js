...
function generateUIBindingEvents(data) {
    ...
    mui('.address-list').on('tap', '.address-list-cell-operation-edit', function() {
        ...
        location.href = 'addressEdit.html?id=' + index + '&sku=' + getQueryString('sku') + '&norder=' + getQueryString('norder') + '&edit=true';
    });
    ...
    mui('.address-list').on('tap', '.address-items', function(e) {
        // debugger;
        if (e.target.className != 'address-items-edit address-list-cell-operation-edit' && getQueryString('norder') != '1') {
            // debugger;
            var addressId = this.getAttribute('index');
