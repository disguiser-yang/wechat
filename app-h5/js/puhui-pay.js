    ...
    mui.post('/puhuihua/wechat/wallet/hasPayPassword', {
        ...
        if (data.success) {
            if (!data.data) {
                ...
                $('.pay-qr-code-wrap').show();
                $('#wallet-infor').show();
                $('.refresh-btn').css('visibility', 'visible');
                // $('#open-puhui-wallet').show();
                //判断是否免密
                mui.post('/puhuihua/wechat/wallet/hasPass', {
                    token: token + '_' + terminal
...
//显示密码
mui('body').on('tap', '.show-pwd', function() {
    if ($(this).hasClass('show-pwd-span')) {
        $(this).removeClass('show-pwd-span');
        $('.pwd-row input').attr('type', 'password');
    } else {
        $(this).addClass('show-pwd-span');
        $('.pwd-row input').attr('type', 'text');
    }

});

//立即开通
mui('body').on('tap', '.redirect-open', function() {

    $('#open-puhui-wallet').hide();
    $('#set-pay-pwd').show();
});

//确认开通
mui('body').on('tap', '.comfirm-open', function() {
    if (count < 6) {
        mui.toast('请输入6位支付密码')
    } else {
        $.post('/puhuihua/wechat/wallet/setPayPassword', {
            ...
            if (data.success) {
                $('#set-pay-pwd').hide();
                $('#open-puhui-wallet-success').show();
            } else if (data.errorCode == 0) {
                mui.toast('网络开小差了');
            } else {
                mui.toast('开通失败');
...
//完成
mui('body').on('tap', '.open-btn-end', function() {

    $('#open-puhui-wallet-success').hide();
    $('.pay-qr-code-wrap').show();
    $('#wallet-infor').show();
    $('.refresh-btn').css('visibility', 'visible');
