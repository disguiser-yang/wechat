        ...
        mui.post('/puhuihua/wechat/userInfo/login', {
            ...
            if (data.success) {
                ...
                if (keyWord[1] == 'url') {
                    ...
                    } else if (isEnter == 'wine') {
                        window.location.replace(wineIndex); //跳转到五粮液订单页面
                    } else if (isEnter == 'questionnaire') {
                        window.location.replace('../../questionnaire/view/index.html'); //跳转到五粮液订单页面
                    } else {
                        switch (page) {
                            case '0':
...
$(function() {
    ...
    var searchKey = window.location.search;
    var isEnter = searchKey.split("=")[1];
    if (isEnter != 'wine' && isEnter != 'questionnaire') {
        console.log('1')
        pushHistory();
